...
def set_handler(levels):
    if levels == 'error':
        logger.addHandler(LogUtil.err_handler)
    logger.addHandler(LogUtil.handler)


# 在记录日志之后移除句柄
def remove_handler(levels):
    if levels == 'error':
        logger.removeHandler(LogUtil.err_handler)
    logger.removeHandler(LogUtil.handler)


def get_current_time():
    return time.strftime(LogUtil.date, time.localtime(time.time()))
...
if __name__ == "__main__":
    ...
    LogUtil.warning("This is warning message")
    LogUtil.error("This is error")
    LogUtil.critical("This is critical message")